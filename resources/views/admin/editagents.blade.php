          ...
          <div class="col-md-12">
            <form method="post" name="frm_edit_category">
              ...
              <label for="packagename">Name</label>
                  <input type="text" id="name" name="name" placeholder="name" style="color:#000000;"  value="{!! $agentsdetail[0]->name !!}" readonly />	
				</div>	
				<div class="col-md-4">
				  <label for="packagename">Category Name</label>					
                  	<select id="catname" name="catid">
					<option value="0" <?php if($agentsdetail[0]->catid ==0) { echo 'selected'; } ?> >Select category</option>
					@foreach($agnetscats as $agnetscat)	
					<option value="{!! $agnetscat->cid !!}" <?php if($agentsdetail[0]->catid ==$agnetscat->cid) { echo 'selected'; } ?>>{!! $agnetscat->catname !!}</option>
					@endforeach
					</select>
				</div>
