	...
	public function newsearchtours(Request $request)
	{
		...
		if($tour_name_location != "") 
		{
			if($tours_month != "") 
			{
				...
				->orderBy('days', 'asc')->orderBy('tour_name', 'asc')->paginate(5);
				$tour_package_type = \DB::select("SELECT tour_package_type,MAX(person) AS persons FROM tours_price GROUP BY tour_package_type");                 
				...
				foreach($tour_package_type as $type)
				{
					...
					->where('tour_package_type',$type->tour_package_type)->where('person',$type->persons)->get()->toArray();
					$tour_prices[] = $data[0];
				}
			}
			else
		...
		".$month_sql." ".$name_location_sql);
		*/
		return view('newtours_search',array('tours'=>$tours,'tours_month'=>$tours_month,'search_tour'=>$tour_name_location,'tour_prices' => $tour_prices));
